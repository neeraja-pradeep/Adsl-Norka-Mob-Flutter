...
class _PaymentSuccessPageState extends State<PaymentSuccessPage>
  ...
  bool _isVerifying = true; // Show verifying state initially
  String _enrollmentNumber = 'NC1234567890'; // Default fallback
  ...
  void initState() {
    ...
    // Delay showing Enroll Now button for 3 seconds
          ...
          child: Column(
            children: [
              ...
              FadeTransition(
                ...
                child: AppText(
                  text: _isEnrollmentComplete
                      ? 'Enrollment number: $_enrollmentNumber'
                      : _isVerifying
              ...
              FadeTransition(
                ...
                child: Column(
                  children: [
                    ...
                    // Show "Download E-Card" and "Back to Home" buttons after enrollment is complete
                    if (_isEnrollmentComplete) ...[
                      CustomButton(
                        text: 'Download E-Card',
                        onPressed: _handleViewECard,
                      ...
                      const SizedBox(height: 12),
                      CustomButton(
                        text: _isLoading ? 'Processing...' : 'Back to Home',
                        onPressed: _isLoading ? () {} : _handleBackToHome,
                        color: AppConstants.primaryColor,
                        textColor: AppConstants.whiteColor,
                        height: 50,
                      ),
  ...
  Future<void> _handleEnrollNow() async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('Request ID Details: ${verificationProvider.requestIdDetails}');
      } catch (e) {
        debugPrint('❌ Error generating request ID: $e');
        ToastMessage.failedToast('Failed to generate request ID');
        setState(() {
          _isEnrolling = false;
        });
        return;
      ...
      // Step 2: Call the enrollment process
      debugPrint('=== STEP 2: STARTING ENROLLMENT PROCESS ===');
      await _handleEnrollmentProcess();

    } catch (e) {
      debugPrint('❌ Enrollment error: $e');
      ToastMessage.failedToast('Enrollment failed. Please try again.');
      if (mounted) {
        setState(() {
          _isEnrolling = false;
        });
