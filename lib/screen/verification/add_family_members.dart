...
class _AddFamilyMembersState extends State<AddFamilyMembers>
  ...
  int _calculateAge(String dob) {
    ...
    try {
      ...
      // Date picker formats dates as DD/MM/YYYY, so always parse / as DD/MM/YYYY
      if (dob.contains('/')) {
        // Always DD/MM/YYYY format from date picker
        day = int.parse(parts[0]);
        month = int.parse(parts[1]);
        year = int.parse(parts[2]);
      } else if (dob.contains('-')) {
        // For hyphens, check if first part is 4 digits (year) - YYYY-MM-DD format
        if (parts[0].length == 4) {
      ...
      // Create DateTime object - this will throw if date is invalid (e.g., Feb 30)
      DateTime birthDate = DateTime(year, month, day);
      DateTime currentDate = DateTime.now();
      
      // Double-check: verify the parsed values match the DateTime (handles invalid dates like Feb 30)
      ...
      // Adjust age if birthday hasn't occurred this year (calculate completed years)
      if (currentDate.month < birthDate.month ||
          (currentDate.month == birthDate.month && currentDate.day < birthDate.day)) {
        age--;
      }
      
      // Ensure age is not negative (shouldn't happen due to future date check, but safety check)
      if (age < 0) {
        return 0;
      }
      ...
      debugPrint('Error calculating age: $e for DOB: $dob');
      return 0;
    }
  }

  // Calculate age for child (allows future dates - minimum is today)
  int _calculateAgeForChild(String dob) {
    if (dob.isEmpty) return -1; // Return -1 for invalid to distinguish from valid 0
    ...
    try {
      ...
      // Allow future years for children
      if (year < 1900) {
        return -1;
      }
      ...
      // Double-check: verify the parsed values match the DateTime
      if (birthDate.year != year || birthDate.month != month || birthDate.day != day) {
        return -1;
      }
      
      // Calculate age (allows future dates - will be negative if future)
      int age = currentDate.year - birthDate.year;
      ...
      // For children, we allow negative ages (future dates) and 0 (today)
      // Negative age means future date, which is allowed for children
      return age;
    } catch (e) {
      debugPrint('Error calculating child age: $e for DOB: $dob');
      return -1;
    }
  }
  ...
  void _addFamilyMember() {
    ...
    // Validate age based on relationship
    if (relationship == 'Spouse') {
      // Check if age calculation failed
      if (age <= 0) {
        ToastMessage.failedToast('Invalid date of birth. Please check the date and try again.');
        return;
      }
      // Spouse must have completed 18 years (age must be >= 18, reject if <= 17)
      if (age < 18) {
        ToastMessage.failedToast('Spouse must have completed 18 years of age');
        return;
      }
      // Spouse maximum age is 70 (reject if > 70)
      if (age > 70) {
        ToastMessage.failedToast('Spouse age cannot exceed 70 years');
        return;
      }
    } else if (relationship.startsWith('Child')) {
      // For children, allow age >= 0 (includes today's date and future dates)
      // Calculate age allowing future dates
      int childAge = _calculateAgeForChild(_dateOfBirthController.text);
      
      // Check if age calculation failed (invalid date format)
      // -1 means invalid date, any other value (including negative for future dates) is valid
      if (childAge == -1) {
        ToastMessage.failedToast('Invalid date of birth. Please check the date and try again.');
        return;
      }
      
      // Child minimum age is 0 (today's date is allowed - age = 0)
      // Negative age means future date, which is allowed for children
      // Age 0 = born today (allowed)
      // Age > 0 = born in the past (allowed up to 25)
      // Child maximum age is 25 (reject if > 25)
      if (childAge > 25) {
        ToastMessage.failedToast('Child age cannot exceed 25 years');
        return;
      }
      // Age 0 and negative ages (future dates) are allowed - no minimum age restriction
    }
  ...
  Future<void> _proceedToApp() async {
    ...
    try {
      ...
      // Validate self age (must have completed 18 years and ≤ 70 years)
      int selfAge = _calculateAge(_customerDateOfBirthController.text);
      // Check if age calculation failed
      if (selfAge <= 0) {
        ToastMessage.failedToast(
          'Invalid date of birth. Please check your date of birth and try again.',
        );
        setState(() {
          _isLoading = false;
        });
      ...
      // Self must have completed 18 years (age must be >= 18, reject if < 18)
      if (selfAge < 18) {
        ToastMessage.failedToast(
          'You must have completed 18 years of age to continue.',
        );
        setState(() {
          _isLoading = false;
        });
      ...
      // Self maximum age is 70 (reject if > 70)
      if (selfAge > 70) {
        ToastMessage.failedToast(
          'Sorry, you cannot continue as your age exceeds 70 years. Maximum age limit is 70 years.',
        );
      ...
      // Validate spouse age (must have completed 18 years and ≤ 70 years)
      final spouse = _familyMembers.firstWhere(
        (member) => member.relationship == 'Spouse',
        orElse: () =>
            FamilyMember(name: '', relationship: '', dateOfBirth: '', gender: ''),
      ...
      if (spouse.name.isNotEmpty) {
        int spouseAge = _calculateAge(spouse.dateOfBirth);
        // Check if age calculation failed
        if (spouseAge <= 0) {
          ToastMessage.failedToast(
            'Invalid spouse date of birth. Please check and try again.',
          );
          setState(() {
            _isLoading = false;
          });
        ...
        // Spouse must have completed 18 years (age must be >= 18, reject if < 18)
        if (spouseAge < 18) {
          ToastMessage.failedToast(
            'Spouse must have completed 18 years of age',
          );
          setState(() {
            _isLoading = false;
          });
      ...
      // Validate children ages (minimum is today's date, maximum is 25 years)
      final children = _familyMembers
          .where((member) => member.relationship.startsWith('Child'))
          .toList();
      
      for (var child in children) {
        int childAge = _calculateAgeForChild(child.dateOfBirth);
        // Check if age calculation failed (invalid date format)
        // -1 means invalid date, any other value (including negative for future dates) is valid
        if (childAge == -1) {
          ToastMessage.failedToast(
            'Invalid child date of birth for ${child.name}. Please check and try again.',
          );
          setState(() {
            _isLoading = false;
          });
        ...
        // Child minimum age is 0 (today's date or future is allowed)
        // Child maximum age is 25 (reject if > 25)
        if (childAge > 25) {
          ToastMessage.failedToast(
            'Child ${child.name} age cannot exceed 25 years',
          );
          setState(() {
            _isLoading = false;
          });
